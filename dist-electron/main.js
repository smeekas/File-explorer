...
function createWindow() {
  ...
  electron.ipcMain.on("open-new-window", (_, data) => {
    const secondWindow = new electron.BrowserWindow({
      width: 600,
      height: 400,
      parent: win ?? void 0,
      webPreferences: {
...
electron.ipcMain.on(Events.GET_DIRS, async (_, dirPath) => {
  try {
    ...
    const fileData = files.map((fileItem) => {
      return new Promise(
        (resolve, reject) => {
          if (fileItem.isFile()) {
            fs.stat(path.join(fileItem.path, fileItem.name)).then((fileState) => {
              const fileInfo = {
                file: {
                  name: fileItem.name,
            ...
            fs.readdir(path.join(fileItem.path, fileItem.name)).then((dirData) => {
                ...
                (dirInfo2) => {
                  resolve({
                    dir: {
                      ...
                      size: dirData.length,
                      modified: dayjs(dirInfo2.mtime).fromNow()
                    }
                  });
                }
              );
    ...
    win == null ? void 0 : win.webContents.send(Events.GET_DIRS_RESULT, dirInfo);
  } catch (err) {
  }
});
electron.ipcMain.on(Events.GET_DIR_INFO, async (_, dirPath) => {
  const files = await fs.readdir(dirPath);
  win == null ? void 0 : win.webContents.send(Events.GET_DIR_INFO_RESULT, files.length);
});
electron.ipcMain.on(Events.GET_FILE_INFO, async (_, filePath) => {
  const file = await fs.stat(filePath);
  win == null ? void 0 : win.webContents.send(Events.GET_FILE_INFO_RESULT, file);
});
electron.ipcMain.on(Events.GET_HOME_DIR, () => {
  win == null ? void 0 : win.webContents.send(Events.GET_HOME_DIR_RESULT, os.homedir());
});
